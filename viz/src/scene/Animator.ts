...
import type { BuiltCreature } from "./CreatureBuilder";
import type { RunData } from "../io/types";
...
export class Animator {
  private readonly creature: BuiltCreature;
  private readonly run: RunData;
  private readonly nq: number;
  private readonly nFrames: number;
  private readonly frameDt: number;
  ...
  constructor(creature: BuiltCreature, run: RunData) {
    ...
    if (this.creature.joints.length !== run.meta.n_joints) {
      throw new Error(
        `joint count mismatch: creature has ${this.creature.joints.length}, run has ${run.meta.n_joints}`,
      );
    }
  }
  ...
  step(dt: number): void {
    ...
    if (this.time < 0) this.time += total;
  ...
  private applyInterpolated(i0: number, i1: number, t: number): void {
